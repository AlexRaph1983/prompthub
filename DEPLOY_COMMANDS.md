...
```bash
echo "🌍 Проверяем внешний доступ..." && curl -s http://REDACTED_IP:3000 | head -3 && echo "🔗 Проверяем внешний API:" && curl -s http://REDACTED_IP:3000/api/prompts?limit=2 | head -5 && echo "🎉 ДЕПЛОЙ ЗАВЕРШЕН!"
```
...
- [ ] Сайт отвечает по http://REDACTED_IP:3000  

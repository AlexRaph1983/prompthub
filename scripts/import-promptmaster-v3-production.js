...
async function importPromptmasterV3Prompts() {
  try {
    console.log('🚀 Начинаем импорт новых промптов PromptMaster v3...');

    // Читаем промпты
    const promptsFile = path.join(__dirname, '..', 'promptmaster_v3_prompts_for_import.json');
    ...
    for (const promptData of prompts) {
      try {
        ...
        if (!category) {
          ...
          const categoryMap = {
            'Code': 'Продуктивность',      // Code -> Productivity
            ...
            'Education': 'Продуктивность', // Education -> Productivity
            ...
            'Finance': 'Продуктивность',   // Finance -> Productivity
            'Health': 'Продуктивность'     // Health -> Productivity
          };
        ...
        console.log(`✅ Импортирован: "${promptData.title}" -> ${category.nameRu}`);
    ...
    console.log('🎉 Импорт новых промптов PromptMaster v3 завершен успешно!');

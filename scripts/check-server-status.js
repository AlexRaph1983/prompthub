/**
 * Проверка статуса сервера на продакшене
 */

async function checkServerStatus() {
  console.log('🔍 ПРОВЕРКА СТАТУСА СЕРВЕРА НА ПРОДАКШЕНЕ\n')
  ...
  try {
    ...
    if (uniqueValues.length === 1) {
      ...
      console.log('  🔧 Или проверить, что новый код загрузился на сервере')
    }

    console.log('\n✅ Проверка статуса сервера завершена!')

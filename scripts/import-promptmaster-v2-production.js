...
async function importPromptmasterV2Prompts() {
  try {
    console.log('🚀 Начинаем импорт новых промптов PromptMaster v2...');

    // Читаем промпты
    const promptsFile = path.join(__dirname, '..', 'promptmaster_v2_prompts_for_import.json');
      ...
      try {
        ...
        if (!category) {
          ...
          const categoryMap = {
            ...
            'Creative': 'Письмо',
            'Writing': 'Письмо',
            'Finance': 'Продуктивность',
            'Research': 'Продуктивность',
            'Image': 'Изображения',
            'Video': 'Видео',
            'Health': 'Продуктивность'
          };
    ...
    console.log('🎉 Импорт новых промптов PromptMaster завершен успешно!');
